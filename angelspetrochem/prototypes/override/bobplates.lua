...
if mods["bobplates"] then
  for fluid, vals in pairs(Energy_table) do
    if vals.fv then
      data.raw.fluid[fluid].fuel_value =
        math.floor(vals.fv / 5 + 0.5) * 5 .. "kJ"
      data.raw.fluid[fluid].emissions_multiplier =
        vals.em or data.raw.fluid[fluid].emissions_multiplier or 1
      if vals.turr ~= false then
        table.insert(turret_params, {
          type = fluid,
          damage_modifier = math.floor(
            vals.fv / Energy_table["gas-methane"].fv * 10 + 0.5
          ) / 10
        })
      end
    end
...
if mods["bobplates"] then
  ...
  data.raw.fluid["tungstic-acid"].icons =
...
if mods["bobplates"] then
  if angelsmods.trigger.resin then
    ...
    OV.disable_recipe({"bob-resin-wood", "solid-resin"})
    OV.remove_unlock("plastics", "synthetic-wood")
...
if mods["bobplates"] then
  if data.raw.recipe["pure-water-pump"] then
    ...
    data.raw.recipe["pure-water-pump"].icons =
