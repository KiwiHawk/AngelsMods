...
data.raw["assembling-machine"]["oil-refinery"].icons =
  util.table.deepcopy(data.raw["item"]["oil-refinery"].icons)
...
if angelsmods.trigger.disable_vanilla_chemical_plants then
  ...
  data.raw["assembling-machine"]["chemical-plant"].icons =
    util.table.deepcopy(data.raw["item"]["chemical-plant"].icons)
...
OV.global_replace_icon(
  ...
  {
    "__angelspetrochem__/graphics/icons/liquid-sulfuric-acid.png",
    icon_size = 64
  }
)
...
move_item(
  "explosives",
  "petrochem-solids",
  "b[petrochem-solids-2]-a[explosives]"
)
if angelsmods.trigger.early_sulfuric_acid == true then
...
table.insert(turret_params, {
  type = "liquid-fuel-oil",
  damage_modifier = 1.1
})

-- rocket fuel
OV.patch_recipes({{ --default, has no fluids
  name = "rocket-fuel",
  ingredients = {{"!!"}, {
    type = "item",
...
data.raw["item"]["rocket-fuel"].icon =
  "__angelspetrochem__/graphics/icons/rocket-fuel.png"
