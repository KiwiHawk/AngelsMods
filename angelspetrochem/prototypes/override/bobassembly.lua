...
if mods["bobassembly"] then
  ...
  if not angelsmods.trigger.disable_bobs_chemical_plants then
    ...
    for i, plant in
      pairs({"chemical-plant-2", "chemical-plant-3", "chemical-plant-4"})
    do
      if data.raw["assembling-machine"][plant] then
        data.raw["item"][plant].icons =
        ...
        data.raw["assembling-machine"][plant].icons =
          util.table.deepcopy(data.raw["item"][plant].icons)
        data.raw["assembling-machine"][plant].fast_replaceable_group =
          "chemical-plant"
      end
    end
  end
