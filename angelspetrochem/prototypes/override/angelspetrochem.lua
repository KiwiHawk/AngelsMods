...
-- not bobs mods, should we do something, usual thing is to do nothing...
  ...
  -- no deuterium required, disabling it...
  angelsmods.functions.add_flag("gas-enriched-hydrogen-sulfide", "hidden")
  angelsmods.functions.disable_barreling_recipes(
...
if angelsmods.functions.is_special_vanilla() then
  ...
  angelsmods.functions.add_flag(
    {"solid-calcium-chloride", "gas-phosgene", "gas-ammonium-chloride"},
    "hidden"
  )

  if angelsmods.bioprocessing then
  else
    OV.disable_recipe({"gas-urea"})
    angelsmods.functions.add_flag({"gas-urea", "gas-acetone"}, "hidden")
  end
end
...
if angelsmods.trigger.resin then
  ...
  angelsmods.functions.add_flag(
    {"solid-resin", "liquid-resin", "gas-melamine", "liquid-bisphenol-a"},
    "hidden"
  )
  OV.disable_technology({"resins", "resin-1", "resin-2", "resin-3"})
end
...
if angelsmods.trigger.rubber then
  ...
  OV.disable_technology({"rubbers", "rubber"})
end

if angelsmods.trigger.liquid_ferric_chloride_solution then
else
  OV.disable_recipe({"liquid-ferric-chloride-solution"})
...
if angelsmods.trigger.liquid_cupric_chloride_solution then
  ...
  OV.disable_recipe({"liquid-cupric-chloride-solution"})
...
angelsmods.functions.add_flag(
  {"gas-hydrogen-peroxide", "gas-phosgene"},
  "hidden"
)
OV.disable_recipe({"gas-phosgene"})

-----------------------------------------------------------------------------
-- ACIDS --------------------------------------------------------------------
-----------------------------------------------------------------------------
if angelsmods.trigger.enableacids == false then
  OV.disable_recipe({"liquid-hydrofluoric-acid", "liquid-hydrogen-fluoride"})
  angelsmods.functions.add_flag({"liquid-hydrofluoric-acid"}, "hidden")
end
...
if angelsmods.trigger.gas_hydrogen_fluoride == false then
  ...
  angelsmods.functions.add_flag({"gas-hydrogen-fluoride"}, "hidden")
end