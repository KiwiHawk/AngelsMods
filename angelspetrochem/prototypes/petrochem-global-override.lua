...
local unit_test_functions =
  mods["angelsdev-unit-test"] and require(
    "__angelsdev-unit-test__/unit-test-functions"
  ) or nil
...
for _, recipe in pairs(data.raw.recipe) do
  if not recipe.crafting_machine_tint and recipe_categories[recipe.category] and recipe.hidden ~= true then -- only recipes which do not have a crafting machine tint set -- only recipes that can be crafting in machines that support recipe tints -- only recipes which are not hidden (explicitly set to false, or default=nil)
    if unit_test_functions then -- only log when unit testing is in progress
      unit_test_functions.print_msg(
        string.format(
          "Recipe '%s' does not have a crafting_machine_tint set!",
          recipe.name
        )
      )
    end
