...
if angelsmods.industries.overhaul then
  ...
  OV.patch_recipes({{ --halve -- need to have something as bottom layer
  ...
  if mods["bobplates"] then
    ...
    if data.raw.item["plutonium-239"] then
      ...
      data.raw.item["plutonium-239"].icon_size =
        data.raw.item["plutonium-240"].icon_size
      data.raw.item["plutonium-239"].icons =
        data.raw.item["plutonium-240"].icons
    end
    ...
    angelsmods.functions.add_flag(
      {"plutonium-240", "empty-nuclear-fuel-cell"},
      "hidden"
    )
    ...
    if bobmods.revamp and settings.startup["bobmods-revamp-nuclear"].value then
      ...
      if data.raw.item["deuterium-fuel-cell"] then
        ...
        if data.raw.reactor["nuclear-reactor-3"] then
          data.raw.item["angels-deuterium-fuel-cell"].fuel_category =
            "deuterium"
          OV.add_prereq("bob-nuclear-power-3", "angels-fusion-power-1")
        end
      end --remove them from their individual techs
    else
