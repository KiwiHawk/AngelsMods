if angelsmods.industries.components then
  ...
  if mods["bobassembly"] then
    ...
    if data.raw["assembling-machine"]["assembling-machine-1"].ingredient_count and data.raw["assembling-machine"]["assembling-machine-1"].ingredient_count < 4 then -- required for tier 1 block automation
      data.raw["assembling-machine"]["assembling-machine-1"].ingredient_count =
        4
    end

    if data.raw["assembling-machine"]["assembling-machine-2"].ingredient_count and data.raw["assembling-machine"]["assembling-machine-2"].ingredient_count < 5 then -- required for tier 2 block automation
    ...
    OV.patch_recipes({{
      ...
      ingredients = {{
        ...
        amount = "block-construction-4"
      }, {
        type = "item",
        name = "block-enhancement-2",
        amount = "block-enhancement-4"
      }, {
        type = "item",
        name = "block-mechanical-1",
        amount = "block-mechanical-2"
      }, {
        type = "item",
        name = "block-fluidbox-1",
        amount = "block-fluidbox-2"
      }}
    }, {
    ...
    if settings.startup["bobmods-assembly-electronicmachines"].value then
      OV.patch_recipes({{ -- corresponds to assembling machine 1.5 -- corresponds to assembling machine 2.5 -- corresponds to assembling machine 4.5
