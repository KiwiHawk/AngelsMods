...
if angelsmods.industries.tech then
  ...
  function LSlib_TechCleanup.getAllPrerequisites(technologyName)
    ...
    if LSlib_TechCleanup.prereqcache[technologyName] then
      ...
      -- temporary fill in the prerequisites to prevent stack overflow due to loops in technology tree -- all prerequisites for this tech
      ...
      local prerequisites = {}
      for _, prerequisiteName in
        pairs(data.raw["technology"][technologyName].prerequisites)
      do
        LSlib_TechCleanup.prereqcache[technologyName][prerequisiteName] = true

        local prePrerequisites =
          LSlib_TechCleanup.getAllPrerequisites(prerequisiteName)
        for prePrerequisiteName, _ in pairs(prePrerequisites or {}) do
          LSlib_TechCleanup.prereqcache[technologyName][prePrerequisiteName] =
            true
        end
      end
  ...
  function LSlib_TechCleanup.removeDuplicatePrerequisites(technologyName)
    ...
    if containDuplicates then
      ...
      data.raw["technology"][technologyName].prerequisites =
        util.table.deepcopy(newPrerequisites)
    end
  end
  ...
  function LSlib_TechCleanup.removeHiddenPrerequisites(technologyName)
    ...
    for _, prerequisiteName in
      pairs(data.raw["technology"][technologyName].prerequisites)
    do
      if LSlib_TechCleanup.isHidden(
        prerequisiteName
      ) or not LSlib_TechCleanup.isEnabled(prerequisiteName) then
        containHidden = true
      else
        table.insert(newPrerequisites, prerequisiteName)
      end
  ...
  function LSlib_TechCleanup.removeRedundantPrerequisites(technologyName)
    ...
    local oldPrerequisites =
      util.table.deepcopy(data.raw["technology"][technologyName].prerequisites) -- make it read only
    ...
    for _, prerequisiteName in pairs(oldPrerequisites) do
      ...
      local prePrerequisites =
        LSlib_TechCleanup.getAllPrerequisites(prerequisiteName) or {}

      -- STEP 2a: now we check if any prerequisite of the tech is redundant because
      --          it could be a prerequisite of this prerequisite
      for redundantPrerequisiteIndex, redundantPrerequisiteName in
        pairs(oldPrerequisites)
      do
        if redundantPrerequisiteName ~= prerequisiteName and not removedPrerequisites[redundantPrerequisiteName] and prePrerequisites[redundantPrerequisiteName] then -- no need to check itself -- not already deleted -- must be redundant
          -- STEP 2b: remove the technology prerequisite if the prerequisite is a prerequisite of another technology prerequisite
          data.raw["technology"][technologyName].prerequisites[redundantPrerequisiteIndex]
          = nil -- remove it
