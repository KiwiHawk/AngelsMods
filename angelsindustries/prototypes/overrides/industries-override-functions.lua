...
function angelsmods.functions.AI.pack_replace(
techname,
  old_c,
  new_c --tech tier swapping script (for cleaner code)
)
...
function angelsmods.functions.AI.core_replace(
techname,
  old_c,
  new_c,
  tier -- tech core swapping script
)
  tier = tier or 1
  if old_c == "basic" then
...
function angelsmods.functions.AI.pre_req_replace(
techname,
  old_tech,
  new_tech1,
  new_tech2 -- tech prerequisite replacements
)
...
function angelsmods.functions.AI.set_core(techname, core_n)
  ...
  for _, pack in
    pairs(
      (data.raw.technology[techname] or {
        unit = {
          ingredients = {}
        }
      }).unit.ingredients
...
function angelsmods.functions.AI.core_builder()
  ...
  for rec_4tech in pairs(data.raw.technology) do
    if angelsmods.functions.check_exception(
      rec_4tech,
      angelsmods.industries.tech_exceptions
    ) then
      --personal-equipment and other enhancements take priority
      if string.find(rec_4tech, "module") ~= nil or string.find(
        rec_4tech,
        "equipment"
      ) ~= nil or string.find(rec_4tech, "armor") ~= nil or string.find(
        rec_4tech,
        "axe"
      ) ~= nil or string.find(rec_4tech, "personal") ~= nil then
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-enhance-1", 2)
      --war takes next priority
      elseif string.find(rec_4tech, "military") ~= nil or string.find(
        rec_4tech,
        "laser"
      ) ~= nil or string.find(rec_4tech, "combat") ~= nil or string.find(
        rec_4tech,
        "damage"
      ) ~= nil or string.find(rec_4tech, "shell") ~= nil or string.find(
        rec_4tech,
        "flam"
      ) ~= nil or string.find(rec_4tech, "bullet") ~= nil or string.find(
        rec_4tech,
        "rocket"
      ) ~= nil then
        --exploration is next
      ...
      elseif string.find(rec_4tech, "energy") ~= nil or string.find(
        rec_4tech,
        "power"
      ) ~= nil or string.find(rec_4tech, "cabling") ~= nil or string.find(
        rec_4tech,
        "steam-engine"
      ) ~= nil or string.find(rec_4tech, "heat") ~= nil or string.find(
        rec_4tech,
        "turbine"
      ) ~= nil or string.find(rec_4tech, "reactor") ~= nil or string.find(
        rec_4tech,
        "pole"
      ) ~= nil or string.find(rec_4tech, "substation") ~= nil or string.find(
        rec_4tech,
        "boiler"
      ) ~= nil then
        --logistics is next
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-energy-1", 2)
      elseif string.find(rec_4tech, "insert") ~= nil or string.find(
        rec_4tech,
        "logistic"
      ) ~= nil or string.find(rec_4tech, "rail") ~= nil or string.find(
        rec_4tech,
        "train"
      ) ~= nil or string.find(rec_4tech, "braking") ~= nil or string.find(
        rec_4tech,
        "robot"
      ) ~= nil or string.find(rec_4tech, "fluid") ~= nil or string.find(
        rec_4tech,
        "warehouse"
      ) ~= nil then
        --production is up next
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-logistic-1", 2)
      elseif string.find(rec_4tech, "processing") ~= nil or string.find(
        rec_4tech,
        "automation"
      ) ~= nil or string.find(rec_4tech, "plastic") ~= nil or string.find(
        rec_4tech,
        "mining"
      ) ~= nil or string.find(rec_4tech, "research") ~= nil or string.find(
        rec_4tech,
        "battery"
      ) ~= nil or string.find(rec_4tech, "electronic") ~= nil then
        --smelting and metallurgy
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-processing-1", 2)
      elseif string.find(rec_4tech, "smelting") ~= nil or string.find(
        rec_4tech,
        "casting"
      ) ~= nil or string.find(rec_4tech, "metallurgy") ~= nil or string.find(
        rec_4tech,
        "cool"
      ) ~= nil then
        --Bioprocessing updates
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-processing-1", 2)
      elseif string.find(rec_4tech, "bio") ~= nil or string.find(
        rec_4tech,
        "farm"
      ) ~= nil or string.find(rec_4tech, "arbor") ~= nil or string.find(
        rec_4tech,
        "cool"
      ) ~= nil or string.find(rec_4tech, "garden") ~= nil then
        --more war updates
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-processing-1", 2)
      elseif string.find(rec_4tech, "wall") ~= nil or string.find(
        rec_4tech,
        "gate"
      ) ~= nil or string.find(rec_4tech, "shoot") ~= nil or string.find(
        rec_4tech,
        "turret"
      ) ~= nil or string.find(rec_4tech, "explo") ~= nil then
        --petrochem and refining updates
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-war-1", 2)
      elseif string.find(rec_4tech, "chemistry") ~= nil or string.find(
        rec_4tech,
        "ore"
      ) ~= nil or string.find(rec_4tech, "crack") ~= nil or string.find(
        rec_4tech,
        "science-pack"
      ) ~= nil or string.find(rec_4tech, "water") ~= nil then
        angelsmods.functions.AI.set_core(rec_4tech, "datacore-processing-1", 2)
      end
    end
  end
...
function angelsmods.functions.AI.pack_count_update(
tech,
  pack,
  count --cheap and cheerful replacement (possibly should use a better script but if it works...)
)
...
function angelsmods.functions.AI.core_tier_upgrade()
  for techname, technology in pairs(data.raw.technology) do
    ...
    local core_name = nil
    for _, pack in
      pairs(technology.unit and technology.unit.ingredients or {})
    do
      local pack_n = pack[1] or pack.name or ""
      if string.find(pack_n, "angels") and string.find(
        pack_n,
        "science"
      ) and string.find(pack_n, "pack") then
        pack_name = pack_n
    ...
    if pack_name then
      if core_name then
        ...
        if string.sub(core_type, 1, 1) == "-" and string.sub(
          core_type,
          -1,
          -1
        ) == "-" then
          ...
          do
            if pack_name == string.format(
              "angels-science-pack-%s",
              pack_color
            ) then
              if tier_up then
              ...
              if research_type ~= "basic" then
                OV.remove_prereq(
                  techname,
                  string.format("tech-%s-packs", pack_color)
                )
                OV.add_prereq(
                  techname,
                  string.format(tech_prereq[pack_color], research_type)
                )
              end
            end
          end
        end
      elseif angelsmods.functions.check_exception(
        techname,
        angelsmods.industries.tech_exceptions
      ) then
...
function angelsmods.functions.AI.replace_blocks_list(
ing_list --specifically build to be used for replace_con_mats function
)
  ...
  for ni, _ in pairs(ing_list) do
    existing[ni] = {
      name = ing_list[ni].name,
      amount = ing_list[ni].amount
    }
  end
  ...
  for n, _ in pairs(ing_list) do
    ...
    if block_replace[nme] then
      ...
      if existing[new] then
        ...
        end --check if replacement is already listed
      else
...
function angelsmods.functions.AI.replace_recipe_ing(
recipe,
  old_ing,
  new_ing,
  new_count
  ...
  if recipe.name then --ensure the recipe exists
    ...
    if recipe.ingredients then
      angelsmods.functions.AI.rec_tab_replace(
        recipe.ingredients,
        old_ing,
        new_ing,
        new_count
