...
if angelsmods.industries.tech then
  ...
  local lab_tiers = {{new_pack = "angels-science-pack-grey"}, {
  ...
  for tier_index, tier_props in pairs(lab_tiers) do
    ...
    if tier_props then
      ...
      lab_entity.max_health =
        lab_entity.max_health + (tier_props.health_inc or 0)
      lab_entity.researching_speed =
        lab_entity.researching_speed * (tier_props.speed_multiplier or 1)
    ...
    lab_item_tier.order =
      lab_item_tier.order .. string.format("-%i", tier_index)
    ...
    lab_entity_tier.localised_description =
      {
        ...
        "\n",
        {
          ...
          create_rich_text_icons(lab_entity_tier.inputs)
        }
      }
    lab_entity_tier.icons = util.table.deepcopy(lab_item_tier.icons)
    lab_entity_tier.minable.result = lab_item_tier.name
    lab_entity_tier.energy_usage =
      string.format("%iMW", lab_entity_tier.researching_speed)

    data:extend({lab_item_tier, lab_entity_tier})
  ...
  --[[
    ...
    localised_description = {
      ...
      "\n",
      {
        ...
        create_rich_text_icons(
          data.raw["lab"][lab_item.name .. string.format("-%i", 1)].inputs
        )
      }
    },
    icon = "__angelsindustries__/graphics/icons/crash-site-lab-repaired.png",
    ...
    max_health = data.raw["lab"][lab_item.name .. string.format(
      "-%i",
      1
    )].max_health / 2,
    corpse = "big-remnants",
    ...
    energy_usage = string.format(
      "%ikW",
      data.raw["lab"][lab_item.name .. string.format(
        "-%i",
        1
      )].researching_speed / 2 * 1000
    ),
    researching_speed = data.raw["lab"][lab_item.name .. string.format(
      "-%i",
      1
    )].researching_speed / 2,
  ...
  angelsmods.triggers.lab_ignore_token[lab_item.name .. string.format("-%i", 0)]
  = true
end