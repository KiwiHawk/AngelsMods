...
for _, centrifuge_name in pairs{"centrifuge", "centrifuge-2", "centrifuge-3"} do
  ...
  if centrifuge then
    ...
    centrifuge.working_visualisations = {-- WORKING LIGHTS
    ...
    }, { -- Centrifuge A (Front Left) -- AREA LIGHT
      effect = "uranium-glow",
      apply_recipe_tint = "primary",
      fadeout = true,
      light = {
...
for _, centrifuge_name in pairs{"centrifuge", "centrifuge-2", "centrifuge-3"} do
  ...
  if centrifuge then
    ...
    if has_fluid_input_box then  -- assume 3x3 entity collision box
    else
    ...
    if has_fluid_output_box then  -- assume 3x3 entity collision box
    else
      table.insert(centrifuge.fluid_boxes, {
...
local bob_centrifuge_2 =
  mods["bobsassembly"] and data.raw["assembling-machine"]["centrifuge-2"] and true or false
local bob_centrifuge_3 =
  bob_centrifuge_2 and data.raw["assembling-machine"]["centrifuge-3"] and true or false
for centrifuge_name, centrifuge_categegories in
  pairs{
    centrifuge = bob_centrifuge_2 and {"centrifuging"} or {
      "centrifuging",
      "centrifuging-2",
      "centrifuging-3"
    },
    ["centrifuge-2"] = bob_centrifuge_3 and {
      "centrifuging",
      "centrifuging-2"
