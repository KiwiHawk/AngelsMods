...
if settings.startup["angels-enable-industries"].value or game.active_mods["bobplates"] then -- overhaul enabled
  for _, force in pairs(game.forces) do
    for newTech, oldTechs in
      pairs{
        ["water-chemistry-1"] = {"heavy-water-processing"},
        ["water-chemistry-2"] = {"deuterium-processing"},
        ["angels-plutonium-power"] = {"plutonium-fuel-cell"},
    ...
    do
      if force.technologies[newTech] then
        for _, oldTech in pairs(oldTechs) do
          force.technologies[newTech].researched =
            force.technologies[newTech].researched or (force.technologies[oldTech] and force.technologies[oldTech].researched)
        end
      end
    end

    for techname, enabled in
      pairs{
        -- deuterium
  ...
  for _, surface in pairs(game.surfaces) do
    ...
    angelsmods.migration.replace_recipes(
      ...
      },
      {
        ...
        not (game.active_mods["bobrevamp"] and settings.startup["bobmods-revamp-rtg"] and settings.startup["bobmods-revamp-rtg"].value) and {
          "bobingabout-process",
          nil
