...
local recipes_to_ignore = {["electric-energy-interface"] = true} -- base game
      ...
      for _, recipe_ingredient in pairs(recipe_ingredients) do
        if recipe_ingredient.type == "item" then
          if item_prototypes[recipe_ingredient.name].has_flag("hidden") then
            unit_test_functions.print_msg(
              string.format(
                "Recipe %q requires %q (item), which is hidden.",
                recipe_name,
        ...
        elseif recipe_ingredient.type == "fluid" then
          if fluid_prototypes[recipe_ingredient.name].hidden then
            unit_test_functions.print_msg(
              string.format(
                "Recipe %q requires %q (fluid), which is hidden.",
                recipe_name,
                recipe_ingredient.name
              )
            )
          ...
          unit_test_functions.print_msg(
            string.format(
              "Unhandled testing case for recipe ingredient type %q!",
        ...
        if not products_to_ignore[recipe_product.name] then
          if recipe_product.type == "item" then
            if item_prototypes[recipe_product.name].has_flag("hidden") then
              unit_test_functions.print_msg(
                string.format(
                  "Recipe %q makes %q (item), which is hidden.",
                  recipe_name,
                  recipe_product.name
                )
              )
          ...
          elseif recipe_product.type == "fluid" then
            if fluid_prototypes[recipe_product.name].hidden then
              unit_test_functions.print_msg(
                string.format(
                  "Recipe %q makes %q (fluid), which is hidden.",
                  recipe_name,
                  recipe_product.name
                )
              )
            ...
            unit_test_functions.print_msg(
              string.format(
                "Unhandled testing case for recipe result type %q!",
                recipe_product.type
              )
            )
            return unit_test_functions.test_invalid
