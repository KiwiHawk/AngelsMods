...
local recipe_categories_to_ignore = {"barreling-pump"}
...
local unit_test_007 = function()
  ...
  table.insert(rocket_silo_filters, {
    filter = "type",
    invert = false,
    mode = "and",
    type = "rocket-silo"
  })
  local rocket_silo_prototypes =
    game.get_filtered_entity_prototypes(rocket_silo_filters)
  ...
  for _, rocket_silo_prototype in pairs(rocket_silo_prototypes) do
    if rocket_silo_prototype.fixed_recipe then
      table.insert(
        rocket_silo_fixed_recipe_names,
        rocket_silo_prototype.fixed_recipe
      )
    else
      for rocket_silo_crafting_category, _ in
        pairs(rocket_silo_prototype.crafting_categories)
      do
  ...
  for rocket_silo_recipe_category, _ in pairs(rocket_silo_recipe_categories) do
    table.insert(rocket_silo_recipe_filters, {
      filter = "category",
      invert = false,
      mode = "or",
      category = rocket_silo_recipe_category
    })
  end
  ...
  if #rocket_silo_recipe_filters > 0 then
    rocket_silo_recipes =
      game.get_filtered_recipe_prototypes(rocket_silo_recipe_filters)
  end
  local recipe_prototypes = game.recipe_prototypes
  for _, rocket_silo_fixed_recipe_name in
    pairs(rocket_silo_fixed_recipe_names)
  do
    table.insert(
      rocket_silo_recipes,
      recipe_prototypes[rocket_silo_fixed_recipe_name]
    ) -- extend the recipe list with the fixed recipes
  end
  for _, rocket_silo_recipe in pairs(rocket_silo_recipes) do
    for _, rocket_silo_product in pairs(rocket_silo_recipe.products) do
      if rocket_silo_product.type == "item" and (rocket_silo_product.amount or (rocket_silo_product.amount_min and rocket_silo_product.probability)) then
  ...
  for item_name, item in pairs(item_prototypes) do
    ...
    if not has_recipe(recipe_filters, item_recipes_to_ignore) then
      unit_test_functions.print_msg(
        string.format(
          "No (useful) recipe is using item %q as an ingredient.",
          item_name
        )
      )
  ...
  for fluid_name, fluid in pairs(fluid_prototypes) do
    ...
    if not has_recipe(recipe_filters, fluid_recipes_to_ignore) then
      unit_test_functions.print_msg(
        string.format(
          "No (useful) recipe is using fluid %q as an ingredient.",
          fluid_name
        )
      )
      unit_test_result = unit_test_functions.test_failed
