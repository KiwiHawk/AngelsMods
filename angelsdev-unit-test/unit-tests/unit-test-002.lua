...
local function log_invalid_science_configuration(
tech_name,
  tech_analysers,
  tech_datacore_T1,
  tech_datacore_T2
  ...
  unit_test_functions.print_msg(
    string.format(
      "Technology %q has an invalid research pack configuration:",
      tech_name
    )
  )
  unit_test_functions.print_msg(
    string.format("  Science analysers: %s", tech_analysers[1] or "None")
  )
  if #tech_analysers > 1 then
    for i = 2, #tech_analysers do
      unit_test_functions.print_msg(
        string.format("                     %s", tech_analysers[i])
      )
    end
  end
  unit_test_functions.print_msg(
    string.format(
      "  Science datacores: %s",
      tech_datacore_T1[1] or tech_datacore_T2[1] or "None"
    )
  )
  if #tech_datacore_T1 > 1 then
    for i = 2, #tech_datacore_T1 do
      unit_test_functions.print_msg(
        string.format("                     %s", tech_datacore_T1[i])
      )
    end
  end
  if #tech_datacore_T2 > 1 or (#tech_datacore_T1 > 0 and #tech_datacore_T2 > 0) then
    for i = #tech_datacore_T1 > 0 and 1 or 2, #tech_datacore_T2 do
      unit_test_functions.print_msg(
        string.format("                     %s", tech_datacore_T2[i])
      )
    end
  end
end
  ...
  for tech_name, tech_prototype in pairs(tech_prototypes) do
    if tech_prototype.hidden == false then
      ...
      for _, tech_ingredient in pairs(tech_ingredients) do
        tech_packs[tech_ingredient.name] =
          tech_packs[tech_ingredient.name] or (tech_ingredient.amount > 0)
      end

      local tech_analysers = {}
      for _, analyser_name in
        pairs({
      ...
      if #tech_analysers > 1 and (#tech_datacore_T1 + #tech_datacore_T2) > 0 then
        log_invalid_science_configuration(
          tech_name,
          tech_analysers,
          tech_datacore_T1,
          tech_datacore_T2
