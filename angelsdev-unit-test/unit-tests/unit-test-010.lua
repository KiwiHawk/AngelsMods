...
local function process_tech(tech)
  ...
  for _, modifier in pairs(tech.effects) do
    if modifier.type == "unlock-recipe" then
      ...
      if recipe.name ~= "empty-barrel" and string.sub(
        recipe.name,
        1,
        6
      ) == "empty-" and string.sub(recipe.name, -7, -1) == "-barrel" then
        skip = true
      end
      ...
      if string.sub(recipe.name, 1, 5) == "fill-" and string.sub(
        recipe.name,
        -7,
        -1
      ) == "-barrel" then
...
local function make_starting_unlocks()
  ...
  for _, recipe in pairs(recipe_prototypes) do
    if recipe.hidden_from_player_crafting then
      ...
      table.insert(starting_tech.effects, {
        type = "unlock-recipe",
        recipe = recipe.name
      })
    end
...
local function add_ignores()
  ...
  if game.active_mods["angelsbioprocessing"] then
    ignored_unlocks["plastics"] = {
      items = {},
      fluids = {"liquid-plastic"}
    }
    ignored_unlocks["rubbers"] = {
      items = {},
      fluids = {"liquid-rubber"}
    }
    ...
    ignored_unlocks["bio-pressing-1"] = {
      items = {"solid-nuts", "solid-pips", "solid-beans"},
      fluids = {}
    }
  ...
  if game.active_mods["angelsindustries"] then
    ignored_unlocks["angels-nuclear-fuel"] = {
      items = {"plutonium-239", "plutonium-240", "thorium-232"},
      fluids = {}
    }
    ignored_unlocks["atomic-bomb"] = {
      items = {"plutonium-239", "plutonium-240", "angels-muon-fusion-catalyst"},
      fluids = {"gas-deuterium"}
    }
  end
...
local unit_test_010 = function()
  ...
  if escape == true then
    unit_test_functions.print_msg(
      "Not all techs were checked. Possibly due to hidden prerequisites"
    )
    unit_test_result = unit_test_functions.test_failed
  end
