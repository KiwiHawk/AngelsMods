...
local suitable_research_lab = function(tech_prototype)
  if not tech_prototype then return true end -- edge case behaviour

  local has_suitable_lab_inputs = function(entity_prototype)
    if not entity_prototype then return end -- edge case behaviour
    if not entity_prototype.lab_inputs then return #tech_prototype.research_unit_ingredients == 0 end
    ...
    for _, lab_input_item_data in pairs(tech_prototype.research_unit_ingredients) do
      ...
      if not entity_has_ingredient then return false end
    end
    ...
    if (not tech_hidden(tech_prototype)) and (not tech_unlocked_by_script[tech_name]) then
      ...
      for prereq_name, prereq_prototype in pairs(tech_prototype.prerequisites) do
        if tech_hidden(prereq_prototype) and (not tech_unlocked_by_script[prereq_name]) then -- tech cannot be researched
          unit_test_functions.print_msg(string.format("Technology %q depends on %q, which is hidden.", tech_name,
            prereq_name))
      ...
      if not suitable_research_lab(tech_prototype) then
        ...
        for _, lab_input_item_data in pairs(tech_prototype.research_unit_ingredients) do
          tech_ingredient_list = string.format("%s%s%q", tech_ingredient_list, tech_ingredient_list == "" and "" or ", "
            , lab_input_item_data.name)
        end
        unit_test_functions.print_msg(string.format("Technology %q cannot be researched, no lab accepts the required inputs (%s)."
          , tech_name, tech_ingredient_list))
        unit_test_result = unit_test_functions.test_failed -- soft failure
      end
    end
  end
