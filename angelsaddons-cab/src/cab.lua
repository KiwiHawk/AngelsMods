...
-------------------------------------------------------------------------------
local energyInterfaceSettings =
  require("prototypes/settings").equipment["energy-interface"]
local energyInterfaceTransferRates = {}
for tier, rate in pairs(energyInterfaceSettings.transferRates) do
  energyInterfaceTransferRates[string.format(
      energyInterfaceSettings.name,
      "-" .. tier
    )]
  = rate
end
...
local tick_getEnergyInterface = function(deployedCabData)
  for _, interfaceName in pairs{"angels-cab-energy-interface-mk1"} do
...
local tick_updateDeployedCab = function(deployedCabData)
  --log("------------------------------------------------------------------------------------------")
  local energyInterfaceName, energyInterface =
    tick_getEnergyInterface(deployedCabData)
...
return {
  ...
  deploy = function(entity)
    ...
    local function cannotDeploy(cab, localisedMessage)
      -- inform player(s)
      for _, entity in pairs{cab.get_driver(), cab.get_passenger()} do
        if entity.is_player() then -- player in godmode
          entity.print(localisedMessage) -- player in character
        elseif entity.type == "character" and entity.player then
    ...
    if global.vehicleData.deployedCabs[identifier] then
      return cannotDeploy(deployedCab["angels-cab"], {
        "angels-cab-messages.deploy-alreadyDeployed"
      })
    end
    ...
    if math.abs(deployedCab["angels-cab"].speed) >= 0.1 / 216 then
      return cannotDeploy(deployedCab["angels-cab"], {
        "angels-cab-messages.deploy-driveSpeed"
      })
    else
    ...
    if not deployedCab["angels-cab"].grid.get_contents()["angels-cab-energy-interface-mk1"] then
      return cannotDeploy(deployedCab["angels-cab"], {
        "angels-cab-messages.deploy-noEnergyInterface",
    ...
    for _, entityName in
      pairs{"angels-cab-energy-interface-mk1", "angels-cab-electric-pole"}
    do
  ...
  undeploy = function(entity)
    ...
    if not global.vehicleData.deployedCabs[identifier] then -- not deployed yet
      return cannotUndeploy(
        deployedCab,
        "angels-cab-messages.undeploy-notDeployed"
      )
    end
    ...
    for _, entityName in
      pairs{"angels-cab-energy-interface", "angels-cab-electric-pole"}
    do
