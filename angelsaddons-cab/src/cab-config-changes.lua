...
return function(configurationData)
  ...
  if modChanges and modChanges.new_version ~= (modChanges.old_version or "") then
    ...
    if global.vehicleData.version == 1.0 then
      ...
      log(
        string.format(
          "    Updating vehicleData from version '%.1f' to version '%.1f'.",
          oldVersion,
          newVersion
        )
      )

      for deployedCabIndex, deployedCabData in
        pairs(global.vehicleData.deployedCabs or {})
      do
        -- Migration of "angels-cab-energy-interface" -> "angels-cab-energy-interface-mk1"
        deployedCabData["angels-cab-energy-interface-mk1"] =
          deployedCabData["angels-cab-energy-interface"]
        deployedCabData["angels-cab-energy-interface"] = nil
      end

      log(
        string.format(
          "    Finished updating vehicleData to version '%.1f'.",
          newVersion
        )
      )
