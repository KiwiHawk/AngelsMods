...
if settings.startup["deco-shred-create-shrine-offer"].value then
  for _, decoName in pairs({"shred", "santa", "inter", "voske", "east"}) do
    ...
    -- recipe
      ...
      order = data.raw["item"]["satellite"].order .. "-a[" .. data.raw["item"]["deco-" .. decoName .. "-1"].order .. "]",
      stack_size = data.raw["item"]["satellite"].stack_size,
      rocket_launch_product = util.table.deepcopy(
