...
remote.add_interface("nilaustruck", {hauler_types = function()
  return {"nilaus-truck"}
end})
...
function get_event_spawn_location(player_location)
  ...
  local spawn_radius =
    (radius_max - radius_min) * math.random() * (offset_radius < 0 and -1 or 1) -- maximum 20+10 tiles away from the player
  ...
  return {
    x = player_location.x + x,
    y = player_location.y + y
  }
end
...
function show_event_stats()
  game.print(
    serpent.block({
      ...
      next_event = string.format(
        "%d minutes",
        ((global.next_event_tick or 0) - game.tick) / 60 / 60
      )
    })
  )
end
...
function execute_event_part1()
  ...
  if player and player.valid and player.connected and player.character and player.character.valid and game.item_prototypes["solid-paper"] and player.get_item_count(
  -- nilaus is ingame and alive! -- TP enabled -- running low -- can insert TP as he is running low
    "solid-paper"
  ) < 1000 and player.can_insert{
    name = "solid-paper"
  } and global.event_unit == -1 and global.event_part < 1 then -- event not in progress
    global.event_part = 1
...
script.on_nth_tick(60 * 5, function(event)
  ...
  if global.next_event_tick == nil then
    ...
    global.event_unit = -1
    global.event_part = 0
    global.event_entity = nil -- not in the first 1.5 hours
  elseif tick > global.next_event_tick then
    execute_event_part1()
