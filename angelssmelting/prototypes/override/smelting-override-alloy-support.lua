...
if mods["bobplates"] then
  for k, v in pairs(data.raw.recipe) do
    if v.category == "mixing-furnace" then --alien-blue-alloy, alien-orange-alloy
      data.raw.recipe[v.name].category = "blast-smelting" -- silicon-nitride, silicon-carbide, lithium-cobalt-oxide
    elseif v.category == "chemical-furnace" then
  ...
  angelsmods.functions.add_flag(
    {"steel-chemical-furnace", "fluid-chemical-furnace"},
    "hidden"
  )
  ...
  data.raw["assembling-machine"]["electric-chemical-furnace"].crafting_categories
  = {"chemical-furnace"}
  ...
  if mods["bobassembly"] and settings.startup["bobmods-assembly-multipurposefurnaces"].value then
    ...
    OV.patch_recipes({{
      ...
      ingredients = {{"electric-mixing-furnace", "electric-furnace"}}
    }})
    ...
    for _, rep in
      pairs({{
        ...
        locale = "angels-stone-ingredient-furnace"
      }, {
        name = "steel-mixing-furnace",
        locale = "angels-steel-ingredient-furnace"
      }, {
        name = "electric-mixing-furnace",
        locale = "angels-electric-ingredient-furnace-1"
      }, {
        name = "electric-chemical-mixing-furnace",
        locale = "angels-electric-ingredient-furnace-2"
      }, {
        name = "electric-chemical-mixing-furnace-2",
        locale = "angels-electric-ingredient-furnace-3"
      }, settings.startup["bobmods-assembly-oilfurnaces"].value and {
        name = "fluid-mixing-furnace",
        locale = "angels-fluid-ingredient-furnace"
      } or nil})
    do
      --if data.raw["assembling-machine"][rep.name] then
      data.raw["assembling-machine"][rep.name].localised_name =
        {"entity-name." .. rep.locale}
      --end
    end
    -- tech tree updates
    ...
    data.raw.technology["multi-purpose-furnace-1"].localised_name =
      {"technology-name.angels-multi-purpose-furnace-1"}
    data.raw.technology["multi-purpose-furnace-2"].localised_name =
      {"technology-name.angels-multi-purpose-furnace-2"} --remove metal mixing furnaces if multi-purpose are also removed
    ...
    angelsmods.functions.add_flag(
      {"steel-mixing-furnace", "fluid-mixing-furnace"},
      "hidden"
    )
