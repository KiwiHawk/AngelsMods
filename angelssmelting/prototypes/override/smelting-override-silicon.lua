...
if angelsmods.trigger.smelting_products["silicon"].ingot then
  if angelsmods.trigger.smelting_products["silicon"].mono or angelsmods.trigger.smelting_products["silicon"].wafer --[[, "angels-silicon-casting-3"]] then
    -- no need for molten recipe
    -- swap tech tier 1 to ingots
  else
    angelsmods.functions.add_flag("liquid-molten-silicon", "hidden")
    OV.disable_recipe({"molten-silicon-smelting"})
    OV.disable_technology({"angels-silicon-casting-2"})
    for _, property in
      pairs({"icon", "icon_size", "icon_mipmaps", "icons", "localised_name"})
    do
      data.raw.technology["angels-silicon-smelting-1"][property] =
        util.table.deepcopy(
          data.raw.technology["angels-silicon-smelting-2"][property]
        )
    end
  end --[[, "angels-silicon-casting-3"]]
else
...
if angelsmods.trigger.smelting_products["silicon"].powder then
  if mods["bobplates"] then
    ...
    data.raw.item["silicon-powder"].icon =
      "__angelssmelting__/graphics/icons/powder-silica.png"
    data.raw.item["silicon-powder"].icon_size = 64
    data.raw.item["silicon-powder"].localised_name =
      {"item-name.powder-silicon"}
    OV.patch_recipes({{
      name = "silicon-powder",
      subgroup = "angels-silicon",
      energy_required = 0.5,
      ingredients = {{"!!"}, {"ingot-silicon", 1}},
