...
return function(spawner_name, spawn_data)
  ...
  -- SPAWN INFO
  local spawn_description =
    {
      "",
      {"tips-and-tricks-description.angels-native-inhabitants-spawner-spawning"}
    }
  for unit_idx, unit_data in pairs(spawn_data) do
    ...
    if unit then
      local spawn_unit =
        {
          "tips-and-tricks-description.angels-native-inhabitants-spawner-spawning-unit"
        }
  ...
  if loot and (#loot > 0) then
    local loot_description =
      {
        "",
        {"tips-and-tricks-description.angels-native-inhabitants-spawner-loot"}
      }
    for _, drop in pairs(loot) do
      ...
      local drop_description =
        {
          "tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item"
        }
      if prob == "100" then
        if min == max then
          table.insert(drop_description, {
            "tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount",
            min
          })
        else
          table.insert(drop_description, {
            "tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-amount-range",
            min,
            max
          })
        end
        ...
        if min == max then
          if max == "1" then
            table.insert(drop_description, {
              "tips-and-tricks-description.angels-native-inhabitants-spawner-loot-item-percentage",
              prob
            })
          else
            local drop_combo =
