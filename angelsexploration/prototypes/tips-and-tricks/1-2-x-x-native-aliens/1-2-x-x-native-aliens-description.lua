...
local function compress_localised_string(localised_string)
  ...
  for idx = 2, #localised_string, 19 do
    ...
    for i = idx, math.min(#localised_string, idx + 19) do
      table.insert(
        compressed_string_section,
        util.table.deepcopy(localised_string[i])
      )
    end
...
return function(spawner_name, unit_name, evolution_factor)
  ...
  if range then
    local range_description =
      {"tips-and-tricks-description.angels-native-inhabitants-unit-attack"}
    table.insert(
      range_description,
      unit.localised_name or {
        "tips-and-tricks-name.angels-native-inhabitants-unit",
        "__ENTITY__" .. unit_name .. "__"
      }
    )
    if range > 1 then
      table.insert(range_description, {
        "tips-and-tricks-description.angels-native-inhabitants-unit-attack-long",
        tnt.number_to_string(range)
      })
    else
      table.insert(range_description, {
        "tips-and-tricks-description.angels-native-inhabitants-unit-attack-short",
        tnt.number_to_string(range)
      })
    end
  ...
  if loot and (#loot > 0) then
    local loot_description =
      {"", {"tips-and-tricks-description.angels-native-inhabitants-unit-loot"}}
    for _, drop in pairs(loot) do
      ...
      local drop_description =
        {"tips-and-tricks-description.angels-native-inhabitants-unit-loot-item"}
      if prob == "100" then
        if min == max then
          table.insert(drop_description, {
            "tips-and-tricks-description.angels-native-inhabitants-unit-loot-item-amount",
            min
          })
        else
          table.insert(drop_description, {
            "tips-and-tricks-description.angels-native-inhabitants-unit-loot-item-amount-range",
            min,
            max
          })
        end
        ...
        if min == max then
          if max == "1" then
            table.insert(drop_description, {
              "tips-and-tricks-description.angels-native-inhabitants-unit-loot-item-percentage",
              prob
            })
          else
            local drop_combo =
      ...
      table.insert(
        drop_description,
        (data.raw.item[drop.item] or {}).localised_name or {
      ...
      table.insert(
        loot_description,
        compress_localised_string(drop_description)
      )
    end
