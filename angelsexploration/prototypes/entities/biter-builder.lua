...
local function enemy_autoplace(params)
  ...
  local distance_outside_starting_area =
    noise.var("distance") - noise.var("starting_area_radius")
  ...
  local distance_height_multiplier =
    noise.max(
      0,
      1 + (distance_outside_starting_area - distance_unit * distance_factor) * 0.002 * distance_factor
    )

  local probability_expression =
    noise.var("enemy_base_probability") * distance_height_multiplier
  -- limit probability so that it never quite reaches 1,
  -- because that would result in stupid-looking squares of biter bases:
  probability_expression =
    noise.min(probability_expression, 0.25 + distance_factor * 0.05)
...
local function make_attack_parameter(data_app, data_dmg)
      ...
      action = {
        ...
        action_delivery = {
          ...
          target_effects = {{
            type = "damage",
            damage = {
              amount = data_dmg.damage,
              type = "physical"
            }
          }, {
            type = "damage",
            damage = {
              amount = data_dmg.damage2,
              type = "acid"
            }
          }}
        }
  ...
  if data_app.type == "spitter" then
    ...
    data_dmg.damage_modifier =
      (data_dmg.damage_modifier or 1) * (data_dmg.damage / 50) -- this will make sure damage/sec equals data_dmg.damage
...
function angelsmods.functions.make_alien(def_data)
  ...
  if def_data ~= nil then
    local c_name =
      def_data.appearance.name .. "-" .. def_data.appearance.type .. "-corpse"
    ...
    if def_data.appearance.type == "scarab" then
      data.raw.corpse[c_name].animation =
        make_die_animation(def_data.appearance)
    elseif def_data.appearance.type == "biter" then
      ...
      --psyker and spitter share spitter ones, any custom ones would have to be isolated or paired with these
    ...
    data:extend({{
      ...
      collision_box = {
        {-0.4 * box_scale, -0.4 * box_scale},
        {0.4 * box_scale, 0.4 * box_scale}
      },
      selection_box = {
        {-0.7 * box_scale, -1.5 * box_scale},
        {0.7 * box_scale, 0.3 * box_scale}
      },
      attack_parameters = make_attack_parameter(
        def_data.appearance,
        def_data.attack
      ),
      vision_distance = 30,
...
function angelsmods.functions.update_alien(ua_data)
  ...
  if data.raw.unit[u_name] then
    ...
    unit.attack_parameters =
      make_attack_parameter(ua_data.appearance, ua_data.attack)
...
function angelsmods.functions.update_spawner(us_data)
  ...
  if data.raw["unit-spawner"][s_name] then
    ...
    for _, new_resistance_data in pairs(us_data.resistance or {}) do
      ...
      for old_resistance_index, old_resistance_data in
        pairs(spawner.resistances)
      do
    ...
    for _, new_result_unit_data in pairs(us_data.results or {}) do
      ...
      for old_result_unit_index, old_result_unit_data in
        pairs(spawner.result_units)
      do
...
function angelsmods.functions.compile_alien_data() -- creates an overview of the current alien data
  ...
  local calculate_spawn_data = function(raw_spawn_points)
    local spawn_points = {}
    for _, spawn_point in
      pairs(raw_spawn_points) -- convert indices to numbers
    do
      table.insert(spawn_points, util.table.deepcopy(spawn_point))
    end

    local _, first_spawn_point = next(spawn_points)
    if (first_spawn_point.evolution_factor or first_spawn_point[1]) ~= 0 and (first_spawn_point.spawn_weight or first_spawn_point[2]) ~= 0 then
      ...
      if (#res % 2) == 0 then -- looking for a weight > 0
        if (spawn_point.spawn_weight or spawn_point[2]) ~= 0 then
          if spawn_idx == 1 then
            ...
            table.insert(
              res,
              spawn_points[spawn_idx - 1].evolution_factor or spawn_points[spawn_idx - 1][1]
            )
          end
        end -- looking for a weight == 0
      else
    ...
    for _, spawn in pairs(spawner.result_units) do
      ...
      if data.raw["unit"][spawn_name] then
        spawners[spawner.name][spawn_name] =
          combine_spawn_data(
            spawners[spawner.name][spawn_name],
            calculate_spawn_data(spawn.spawn_points or spawn[2])
          )
        --log(spawn_name)
      else
      end
  ...
  for unit, spawn_data in pairs(units) do
    ...
    for spawner, evo_factor in pairs(spawn_data) do
      if evo_factor == lowest_evo_factor then
        local evo_factor_2 =
          #spawners[spawner][unit] > 1 and spawners[spawner][unit][2] or 1
    ...
    for spawner, evo_factor in pairs(spawn_data) do
      if evo_factor == lowest_evo_factor then
        ...
        if evo_factor_2 < highest_evo_factor_2 or picked then
          table.insert(spawners_to_remove, spawner) -- picking the first if multiple spawners are 'equal'
        else
          picked = true
        end
      end
  ...
  order_spawners = function(spawners)
    ...
    for spawner, spawner_data in pairs(ordered_spawners) do
      ...
      if evo_factor == highest_evo_factor then
        ...
        table.insert(ordered_spawners, {
          [spawner] = util.table.deepcopy(spawner_data)
        })
