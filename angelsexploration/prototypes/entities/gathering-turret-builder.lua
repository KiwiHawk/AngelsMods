...
function angelsmods.functions.create_gathering_turret_base(inputs)
  ...
  if inputs.type and type(inputs.type) == "table" then
    ...
    for _, type in pairs(inputs.type) do
      ...
      local table_picture =
        angelsmods.functions.create_gathering_turret_base(table_inputs)
  ...
  if inputs.type == "gun" then
    return {
      layers = {{
        ...
        tint = inputs.fixed_tint or (not inputs.apply_runtime_tint and {
          r = 0.87,
          g = 0.5,
          b = 0.13,
          a = 0.5
  ...
  if unit.dying_trigger_effect then
    if unit.dying_trigger_effect.type then -- single existing effect
      unit.dying_trigger_effect =
        ...
        } -- table of existing effects
    else
      table.insert(
        unit.dying_trigger_effect,
        angelsmods.functions.create_gathering_turret_start_trigger(inputs)
      )
    end -- no existing effects
  else
    unit.dying_trigger_effect =
      angelsmods.functions.create_gathering_turret_start_trigger(inputs)
  end
...
local function create_sprite_from_icon_layers(icon_layers, icon_scale)
  ...
  for _, icon_layer in pairs(icon_layers) do
    table.insert(
      sprite.layers,
      create_sprite_layer_from_icon_layer(icon_layer, icon_scale)
    )
  end
  return sprite
end
...
local function create_gathering_turret_target_tech(inputs)
  if type(inputs.require_tech_unlock) == "boolean" then
    inputs.require_tech_unlock =
      "angels-gathering-turret-target[" .. inputs.name .. "]"
  ...
  if not tech_data then
    data:extend({{
      ...
      localised_name = {
        "technology-name.angels-gathering-turret-target",
        "__ITEM__" .. inputs.name .. "__"
      },
      icon = "__angelsexploration__/graphics/technology/gathering-turret-target.png",
      icon_size = 256,
      icon_mipmaps = 4,
      prerequisites = inputs.additional_tech_prerequisites and (type(
        inputs.additional_tech_prerequisites
      ) == "string" and {inputs.additional_tech_prerequisites} or type(inputs.additional_tech_prerequisites) == "table" and inputs.additional_tech_prerequisites) or nil,
      effects = {},
      unit = {
        count = 150,
        ingredients = {{
  ...
  table.insert(tech_data.effects, {
    ...
    effect_description = {
      "effect-description.angels-gathering-turret-target",
      "__ITEM__" .. inputs.name .. "__"
    }
  })
end
...
function angelsmods.functions.create_gathering_turret_target(inputs)
  ...
  --dying_explosion = "land-mine-explosion", -- time between placing and the landmine being armed (in ticks)
    type = "land-mine",
    ...
    picture_safe = create_sprite_from_icon_layers(
      angelsmods.functions.get_object_icons(inputs.name),
      0.5
    ),
    picture_set = create_sprite_from_icon_layers(
      angelsmods.functions.get_object_icons(inputs.name),
      0.5
    ),
    picture_set_enemy = create_sprite_from_icon_layers(
      angelsmods.functions.get_object_icons(inputs.name),
      0.5
    ),
    timeout = 0,
...
function angelsmods.functions.generate_gathering_turret_speed_upgrade_technology(
inputs
)
  ...
  return {
    ...
    effects = {inputs.modifier and (inputs.modifier > 0) and {
      type = "ammo-damage",
    ...
    unit = {
      ...
      ingredients = {inputs.level > 0 and {
        type = "item",
        name = "automation-science-pack",
        amount = 1
      } or nil, inputs.level > 0 and {
        type = "item",
        name = "logistic-science-pack",
        amount = 2
      } or nil, inputs.level > 0 and {
        ...
        name = "military-science-pack",
        amount = 2
      } or nil, inputs.level > 3 and {
        type = "item",
        name = "chemical-science-pack",
        amount = 1
      } or nil, inputs.level > 5 and {
        type = "item",
