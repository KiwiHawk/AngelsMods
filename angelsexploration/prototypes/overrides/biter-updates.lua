    ...
    do
      for bob_spawn_idx, bob_spawn_data in pairs(bob_spawner.result_units) do
        if (bob_spawn_data.unit or bob_spawn_data[1]) == unit then
          ...
          for bob_point_idx, bob_point_data in
            pairs(bob_spawn_data.spawn_points or bob_spawn_data[2])
          do
            if first_spawn and (bob_point_data.spawn_weight or bob_point_data[2]) > 0 then
              for base_spawn_idx, base_spawn_data in
                pairs(base_spawner.result_units)
              do
                if (base_spawn_data.unit or base_spawn_data[1]) == unit then
                  for base_point_idx, base_point_data in
                    pairs(base_spawn_data.spawn_points or base_spawn_data[2])
                  do
                    if first_spawn and (base_point_data.spawn_weight or base_point_data[2]) > 0 then
                      local base_point =
                        (base_spawn_data.spawn_points or base_spawn_data[2])[base_point_idx > 1 and (base_point_idx - 1) or 1]
...
local function update_loot_table(loot_table)
  for _, loot in pairs(loot_table or {}) do
    if loot_to_be_converted[loot.item] then
      ...
      loot.count_max =
        ((loot.count_max == nil and 1) or loot.count_max) * loot_to_be_converted[loot.item].rate
