...
function tips_and_tricks_triggers:init_global_data()
  local TNT_data = {
    ...
    prototype_data = {trigger_data = self:init_trigger_data()}, -- data storing info about the trigger prototypes
...
function tips_and_tricks_triggers:init_trigger_data()
  ...
  for alien_name, alien_prototype_data in pairs(alien_data) do
    local technology_name =
      "tips-and-tricks-trigger[angels-native-inhabitants-" .. (alien_prototype_data.type == "unit" and "unit" or "spawner") .. "[" .. alien_name .. "]]"
    trigger_data[alien_name] =
      technology_data[technology_name] and technology_name or nil
  end
  return trigger_data
end
...
function tips_and_tricks_triggers:init_force_data(force_name)
  ...
  global.TNT_data.force_data[force_name] = {trigger_data = force_trigger_data}
end
...
function tips_and_tricks_triggers:remove_trigger(force_name, trigger_name)
  if force_name and global.TNT_data and global.TNT_data.force_data[force_name] then
    local force_triggers =
      global.TNT_data.force_data[force_name]["trigger_data"]
...
function tips_and_tricks_triggers:on_entity_died(killed_entity, killing_entity)
  ...
  local trigger_name =
    self:get_trigger_data(killing_force_name)[killed_entity.name]
  if killing_force_name and trigger_name then
    local trigger_technology =
      game.forces[killing_force_name].technologies[trigger_name]
...
function tips_and_tricks_triggers:on_tech_research_finished(
force_name,
  technology_name
)
