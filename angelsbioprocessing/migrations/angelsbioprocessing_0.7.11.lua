...
for index, force in pairs(game.forces) do
  ...
  do
    if (force.technologies[techName] or {}).researched then
      for techToResearch, techToCheck in
        pairs{["bio-swamp-farming-2"] = "bio-swamp-farming-1"}
      do
        if force.technologies[techToResearch] and force.technologies[techToCheck] then
          force.technologies[techToResearch].researched =
            force.technologies[techToCheck].researched or force.technologies[techToResearch].researched
        end
      end
      break
    end
