  ...
  if item then
    ...
    for _, biome_name in pairs({"temperate", "desert", "swamp"}) do
      for tier_number = 1, 5 do
        local recipe =
          data.raw["recipe"][string.format(
            "sorting-%s-%d",
            biome_name,
            tier_number
          )]
        for _, recipe_difficulty in
          pairs({recipe, recipe.normal, recipe.expensive})
        do
          ...
          for _, recipe_difficulty_result in pairs(recipe_difficulty.results) do
            if (recipe_difficulty_result[1] or recipe_difficulty_result.name) == item_name then
              origin_items[string.format("%s-%d", biome_name, tier_number)] =
                true
            end
          end
        end
    ...
    local localised_origin_items =
      {"", {"item-description.crop-farm-processed"}}
...
if angelsmods.triggers.crystals_full == true then
  ...
  angelsmods.functions.add_flag(
    {"crystal-full-harmonic", "crystal-full-blue", "crystal-full-red"},
    "hidden"
  )
end