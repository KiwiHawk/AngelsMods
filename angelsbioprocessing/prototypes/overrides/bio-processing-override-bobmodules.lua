...
if mods["bobmodules"] then
  ...
  if angelsmods.industries and angelsmods.industries.overhaul then
    ...
    data.raw["item"]["module-processor-board-2"].subgroup =
      "module-intermediates-2"
    data.raw["item"]["module-processor-board-3"].subgroup =
    ...
    data.raw["item"]["effectivity-processor-2"].subgroup =
      "module-intermediates-2"
    data.raw["item"]["effectivity-processor-3"].subgroup =
      "module-intermediates-3"
    data.raw["item"]["productivity-processor-2"].subgroup =
      "module-intermediates-2"
    data.raw["item"]["productivity-processor-3"].subgroup =
      "module-intermediates-3"
    data.raw["item"]["pollution-clean-processor-2"].subgroup =
      "module-intermediates-2"
    data.raw["item"]["pollution-clean-processor-3"].subgroup =
      "module-intermediates-3"
    data.raw["item"]["pollution-create-processor-2"].subgroup =
      "module-intermediates-2"
    data.raw["item"]["pollution-create-processor-3"].subgroup =
      "module-intermediates-3"
  end
  ...
  do
    ...
    OV.set_science_pack(
      --module case --module-circuit-board --processor circuit board
      {type .. "-module-5"},
      {"module-case", "module-circuit-board", type .. "-processor"},
      {0, 1, 2}
    )
  end
  for _, type in pairs{"raw-speed", "green", "raw-productivity"} do
    ...
    OV.set_research_difficulty(type .. "-module-5", 60, 200)
    OV.set_science_pack(
      --module case --module-circuit-board
      {type .. "-module-5"},
      {"module-case", "module-circuit-board"},
      {0, 1}
    )
  ...
  data:extend({{
    ...
    icon_size = 256,
    icon_mipmaps = 4,
    prerequisites = {"modules-2", "bio-processing-crystal-full"},
    effects = {{
      type = "unlock-recipe",
  ...
  for i = 1, 3 do
    ...
    do
      for _, ingredient in
        pairs(data.raw.technology[tech_name].unit.ingredients)
      do
  ...
  for i = 4, 8 do
    ...
    data:extend({{ -- extra pollution absorption
    --{type = "item", name = "solder", amount = solder_amount},
      type = "module",
      name = "angels-bio-yield-module-" .. i,
      localised_description = {
        "item-description.angels-bio-yield-module-" .. i
      },
    ...
    angelsmods.functions.add_bio_productivity_module(
      "angels-bio-yield-module-" .. i
    )
    --adding solder when bobplates is active
    if mods["bobplates"] then
      OV.patch_recipes({{
