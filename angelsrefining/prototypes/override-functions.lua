...
local function generate_fill_barrel_icons(fluid, style)
  ...
  elseif fluid.icons and util.combine_icons then
    f_icon = util.combine_icons(f_icon, util.table.deepcopy(fluid.icons), {
      scale = 0.5,
      shift = {4, -8}
    })
  end
...
local function generate_empty_barrel_icons(fluid, style)
  ...
  elseif fluid.icons and util.combine_icons then
    e_icon = util.combine_icons(e_icon, util.table.deepcopy(fluid.icons), {
      scale = 0.5,
      shift = {7, 8}
    })
  end
...
ov_functions.add_prereq = function(
technology,
  prereq --handles tech OR prereq as table
)
...
ov_functions.remove_prereq = function(
technology,
  prereq --handles tech OR prereq as table
)
  if type(technology) == "table" then
...
ov_functions.disable_technology = function(
technology -- disable technology (may be a table containing a list of technologies)
)
  if type(technology) == "table" then
    for tk, tech in pairs(technology) do
...
ov_functions.remove_normal_input = function(
recipe,
  item -- remove item from input of recipe (item may be a table containing a list of items to remove)
)
...
ov_functions.remove_normal_output = function(
recipe,
  item -- remove item from output of recipe (item may be a table containing a list of items to remove)
)
  if type(item) == "table" then
    for _, it in pairs(item) do
...
ov_functions.global_replace_item = function(
old,
  new -- replace all occurrences of old in recipes with new (old may be a table containing a list of items)
)
...
ov_functions.converter_fluid = function(old_fluid_name, new_fluid_name)
  ...
  if angelsmods.trigger.enableconverter then
    local converter_subgroup_name =
      "angels-fluid-converter-" .. (new_fluid.subgroup or "unknown")

    if not data.raw["item-subgroup"][converter_subgroup_name] then
      local fluid_subgroup_data =
        data.raw["item-subgroup"][new_fluid.subgroup or "unknown"]
...
ov_functions.hide_recipe = function(
recipe -- hides recipe (may be a table containing a list of recipes)
)
...
ov_functions.disable_recipe = function(
recipe -- disables recipe (may be a table containing a list of recipes)
)
  if type(recipe) == "table" then
    for _, rec in pairs(recipe) do
      guarantee_subtable(patch_table, rec)
...
-------------------------------------------------------------------------------
ov_functions.global_replace_science_packs = function(
primary_old,
  secondary_old,
  new,
  amount
  ...
  substitution_table.science_packs[primary_old] = {
    add = new,
    amount = amount or 1
  }
...
ov_functions.set_research_difficulty = function(
technology,
  unit_time,
  unit_amount
)
...
ov_functions.set_temperature_barreling = function(
fluid,
  temp,
  min_temp,
  max_temp
...
ov_functions.barrel_overrides = function(
fluid,
  style --Bottling override functions for icons, localisation and tech unlocks
)
  if data.raw.fluid[fluid] then
    ...
    if data.raw.recipe["fill-" .. fluid_s.name .. "-barrel"] then
      ...
      if style == "gas" then
        ...
        ov_functions.remove_unlock(
          "fluid-barrel-processing",
          "fill-" .. fluid_s.name .. "-barrel"
        )
        ov_functions.add_unlock(
          "gas-canisters",
          "fill-" .. fluid_s.name .. "-barrel"
        )
        ov_functions.remove_unlock(
          "fluid-barrel-processing",
          "empty-" .. fluid_s.name .. "-barrel"
        )
        ov_functions.add_unlock(
          "gas-canisters",
          "empty-" .. fluid_s.name .. "-barrel"
...
-------------------------------------------------------------------------------
local function adjust_recipe(
recipe,
  k -- check a recipe for basic adjustments based on tables and make any necessary changes
)
      ...
      for ix, item in pairs(st) do
        ...
        if replace[item.name] then
          if item.probability then
            ...
            end --check both have amount and update old to new
          elseif item.amount ~= replace[item.name].amount then
...
local function adjust_technology(
tech,
  k -- check a tech for basic adjustments based on tables and make any necessary changes
)
  local function override_subtable(
  subtable,
    o_subtable -- handle special case changes (sort of a partial deep copy/overwrite)
  )
  ...
  local dup_table = {}
  local modifications =
    modify_table.technologies[k] and modify_table.technologies[k].unlocks or nil
  local to_remove = {}
  ...
  dup_table = {}
  modifications =
    modify_table.technologies[k] and modify_table.technologies[k].prereqs or nil
  ...
  dup_table = {}
  modifications =
    modify_table.technologies[k] and modify_table.technologies[k].packs or nil
  to_remove = {}
...
ov_functions.execute = function()
  for k, recipe in
    pairs(data.raw.recipe) -- run through all recipes to perform substitutions/overrides
  do
  ...
  for k, tech in
    pairs(data.raw.technology) -- run through all technologies to perform substitutions/overrides
  do
