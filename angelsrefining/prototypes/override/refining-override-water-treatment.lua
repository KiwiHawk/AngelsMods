...
if angelsmods.trigger.water_red_waste == true then
  ...
  if angelsmods.petrochem then
    ...
    OV.remove_prereq(
      "angels-nitrogen-processin-2",
      "angels-sulfur-processing-1"
    )
  end
  if angelsmods.bioprocessing then
    OV.disable_recipe({"puffer-puffing-13"})
  end
end
if angelsmods.trigger.water_green_waste == true then
  ...
  OV.disable_recipe({"green-waste-water-purification"})
  angelsmods.functions.add_flag({"water-green-waste"}, "hidden")
  if angelsmods.bioprocessing then
    OV.disable_recipe({"puffer-puffing-15"})
  end
end
if angelsmods.trigger.water_greenyellow_waste == true then
  ...
  OV.disable_recipe({"greenyellow-waste-water-purification"})
  angelsmods.functions.add_flag({"water-greenyellow-waste"}, "hidden")
  if angelsmods.petrochem then
    OV.disable_recipe({
    ...
    angelsmods.functions.add_flag(
      {"fluorite-ore", "solid-calcium-sulfate"},
      "hidden"
    )
  end
  if angelsmods.bioprocessing then
    OV.disable_recipe({"puffer-puffing-14"})
  end
end
...
if angelsmods.trigger.washing_tech == false then --not angelsmods.smelting then
  ...
  OV.disable_recipe({"seafloor-pump", "washing-plant", "washing-plant-2"})
  angelsmods.functions.add_flag(
    {"seafloor-pump", "washing-plant", "washing-plant-2"},
    "hidden"
  )
  -- disable technology
...
if angelsmods.trigger.salt_production == false then
  if angelsmods.trigger.smelting_products["lithium"].plate ~= true then
    ...
    OV.disable_recipe({"salination-plant", "salination-plant-2"})
    OV.disable_technology("water-treatment-4")
  end
  OV.disable_recipe({"water-saline", "solid-salt-from-saline", "solid-salt"})
end
...
-------------------------------------------------------------------------------
if mods["bobplates"] then
  data.raw.fluid["lithia-water"].icons =
  ...
  data.raw.fluid["lithia-water"].base_color =
    angelsmods.functions.fluid_color("Ws4Tw")
  data.raw.fluid["lithia-water"].flow_color =
    angelsmods.functions.flow_color("Ws4Tw")
...
if angelsmods.trigger.geode_cyan == false then
  ...
  OV.disable_recipe({"geode-cyan-processing", "geode-cyan-liquify"})
end

if angelsmods.trigger.geode_red == false then
  angelsmods.functions.add_flag("geode-red", "hidden")

  OV.disable_recipe({"geode-red-processing", "geode-red-liquify"})
end