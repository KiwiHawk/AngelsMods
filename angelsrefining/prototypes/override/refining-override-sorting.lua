...
local icon_lookup_table_fallback =
  {icon = "__angelsrefining__/graphics/icons/void.png"}
local icon_lookup_table = {
  ["bauxite-ore"] = mods["angelssmelting"] and {
  ...
  ["cobalt-ore"] = mods["angelssmelting"] and {
  ...
  ["gold-ore"] = mods["angelssmelting"] and {
  ...
  ["lead-ore"] = mods["angelssmelting"] and {
  ...
  ["nickel-ore"] = mods["angelssmelting"] and {
  ...
  ["platinum-ore"] = mods["angelssmelting"] and {
    icon = "__angelssmelting__/graphics/icons/ore-platinum.png"
  } or icon_lookup_table_fallback,
  ["rutile-ore"] = mods["angelssmelting"] and {
  ...
  ["silica-ore"] = mods["angelssmelting"] and {
  ...
  ["silver-ore"] = mods["angelssmelting"] and {
  ...
  ["tin-ore"] = mods["angelssmelting"] and {
  ...
  ["tungsten-ore"] = mods["angelssmelting"] and {
  ...
  ["zinc-ore"] = mods["angelssmelting"] and {
...
local create_basic_sorting_localisation = function(
localised_base_name,
  sorting_tier_names,
  sorting_results,
  has_ore
    ...
    if tier == 0 then
      ...
      if resource then
        if resource.localised_description then
          resource.localised_description =
...
local create_sorting_recipes = function(
refinery_product,
  recipe_base_name,
  sorted_ore_results,
  advanced_sorting
  ...
  local tiers =
    advanced_sorting and {"crushed", "powder", "dust", "crystal"} or {
      "crushed",
      "chunk",
      "crystal",
      "pure"
  ...
  for tier, tier_name in pairs(tiers) do
    ...
    local recipe = {
      name = string.format(recipe_base_name, "-" .. tier_name .. "-processing"),
      results = {}
    }
    if angelsmods.trigger.refinery_products[refinery_product] then
      ...
      angelsmods.functions.OV.disable_recipe(
        string.format(recipe_base_name, "-" .. tier_name)
      )
    end
  ...
  if advanced_sorting and not angelsmods.trigger.refinery_products[refinery_product] then
    angelsmods.functions.OV.disable_recipe(
      string.format(recipe_base_name, "sludge")
    )
    angelsmods.functions.OV.disable_recipe(
      string.format(recipe_base_name, "solution")
    )
    angelsmods.functions.OV.disable_recipe(
      string.format(recipe_base_name, "anode-sludge-filtering")
    )
    angelsmods.functions.OV.disable_recipe(
      string.format(recipe_base_name, "anode-sludge")
    )
  end
...
local create_sorting_mix_recipe = function(
recipe_base_name,
  ore_result_products,
  icon_names,
  ingredients_overrides
)
  local recipes = {}
  for recipe_index, ore_result_product in pairs(ore_result_products) do
    local ore_name =
      type(
        ore_result_product
      ) == "table" and (ore_result_product[1] or ore_result_product.name) or ore_result_product
    local ore_amount =
      type(
        ore_result_product
      ) == "table" and (ore_result_product[2] or ore_result_product.amount) or 1
    local recipe = {
      name = string.format(recipe_base_name, recipe_index),
      results = {{"!!"}, {
        type = type(
          ore_result_product
        ) == "table" and ore_result_product.type or "item",
        name = ore_name,
        amount = ore_amount
      }}
    }
    if angelsmods.trigger.ores[get_trigger_name[ore_name] or ore_name] and ore_amount > 0 then
      ...
      if icon_name then
        if type(icon_name) == "table" then
          ...
          recipe.icon =
            string.format("__angelsrefining__/graphics/icons/%s", icon_name)
        end
      end
...
local create_slag_recipes = function(
recipe_base_name,
  ore_result_products,
  recipe_icons
)
  ...
  for recipe_index = 1, 9 do
    ...
    if #locale_index == 2 then
      recipe.localised_name =
        {"recipe-name.slag_processing_2", locale_index[1], locale_index[2]}
    elseif #locale_index == 3 then
      recipe.localised_name =
        {
          "recipe-name.slag_processing_3",
          locale_index[1],
          locale_index[2],
          locale_index[3]
        }
...
-- FERROUS
-- CUPRIC
  merge_table_of_tables{create_sorting_recipes("saphirite", "angelsore1%s", {
    ...
    ["gold-ore"] = not special_vanilla and {0, 0, 1, 1},
    ["cobalt-ore"] = not special_vanilla and {0, 0, 0, 1}
  }), ore_enabled("ferrous") and create_sorting_recipes(
    "ferrous",
    "angelsore8%s",
    {
    ...
    true
  ) or nil, ore_enabled("cupric") and create_sorting_recipes(
    "cupric",
    "angelsore9%s",
    {
      ["!!"] = not special_vanilla and {true, true, true, true},
      ...
      quartz = not (special_vanilla or ore_enabled("platinum-ore")) and {
        0,
        0,
        0,
        1
...
--[[2]]
--[[3]]
  merge_table_of_tables{create_sorting_mix_recipe(
    "angelsore-crushed-mix%i-processing",
    {{
      type = "item",
      name = "iron-ore",
      amount = 4
    }, special_vanilla and "unused" or {
      type = "item",
      name = "copper-ore",
      amount = 4
    }, special_vanilla and {
    ...
    {special_vanilla and {{
      ...
      amount = 1
    }} or nil, nil, special_vanilla and {{
      type = "item",
    ...
    {special_vanilla and {
      ...
      type = "item",
      name = "uranium-ore",
      amount = special_vanilla and 0 or 3
    }, {
      type = "item",
      name = "thorium-ore",
      amount = 3
    }},
    {special_vanilla and angelsmods.functions.add_icon_layer(
    ...
    ) or {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("rutile-ore", 0.5, {10, 10})}, {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("gold-ore", 0.5, {10, 10})}, {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("cobalt-ore", 0.5, {10, 10})}, nil, {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("uranium-ore", 0.5, {10, 10})}, {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("thorium-ore", 0.5, {10, 10})}}
  ), create_sorting_mix_recipe(
    ...
    {special_vanilla and angelsmods.functions.add_icon_layer(
      ...
      tweaked_icon_lookup("uranium-ore", 0.5, {10, 10})
    ) or {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, tweaked_icon_lookup("tungsten-ore", 0.5, {10, 10})}, {{icon = "__angelsrefining__/graphics/icons/sort-icon.png"}, ore_enabled("platinum") and tweaked_icon_lookup("platinum-ore", 0.5, {10, 10}) or nil}, nil}
  )}
)
...
--[[8]]
--[[9]]
  merge_table_of_tables{create_slag_recipes(
    "slag-processing%s",
    {
      ["iron-ore"] = {special_vanilla and 0.8 or 0.4, 0, 0, 0, 0, 0, 0, 0, 0},
      ["angels-iron-pebbles"] = special_vanilla and {
        0,
        0,
        0,
        3 * 0.8,
      ...
      ["copper-ore"] = {
        special_vanilla and 0 or 0.4,
        special_vanilla and 0.8 or 0,
        0,
        0,
        0,
        0,
      ...
      ["angels-copper-pebbles"] = special_vanilla and {
        0,
        0,
        0,
        0,
      ...
      ["manganese-ore"] = {
        ...
        ore_enabled(
          "platinum-ore"
        ) and ore_enabled("chrome-ore") and 0.4 or ore_enabled("chrome-ore") and 0.2 or 0.4,
        0,
        0,
        0
      },
    ...
    },
    {
      angelsmods.functions.create_liquid_recipe_icon(
        {"iron-ore", not special_vanilla and "copper-ore" or nil},
        slag_color
      ),
      angelsmods.functions.create_liquid_recipe_icon(
        {
      ...
      angelsmods.functions.create_liquid_recipe_icon(
        {
          ...
          not ore_enabled(
            "platinum-ore"
          ) and ore_enabled("chrome-ore") and "chrome-ore" or nil
        },
        slag_color
      ),
      ore_enabled(
        "platinum-ore"
      ) and angelsmods.functions.create_liquid_recipe_icon({"platinum-ore", "chrome-ore"}, slag_color) or nil,
      angelsmods.functions.create_liquid_recipe_icon(
        {"cobalt-ore", "tungsten-ore"},
        slag_color
      ),
      angelsmods.functions.create_liquid_recipe_icon(
        {
