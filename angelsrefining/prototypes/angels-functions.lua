...
function angelsmods.functions.add_icon_layer(
icon_layers,
  layers_to_add,
  layer_shift,
  layer_scale
...
function angelsmods.functions.modify_icon_layers(
icon_layers,
  layer_shift,
  layer_scale
)
  return angelsmods.functions.add_icon_layer(
    {},
    icon_layers,
    layer_shift,
    layer_scale
...
local function unify_tint(tint)
  ...
  if tint then
    ...
    unified_tint.a =
      unified_tint.a > 1 and unified_tint.a / 255 or unified_tint.a
...
function angelsmods.functions.add_number_icon_layer(
icon_layers,
  number_tier,
  number_tint,
  outline_tint
  ...
  elseif icon_layers[1].scale then
    icon_size_scale =
      (icon_layers[1].icon_size or 32) * icon_layers[1].scale / 32
  end
  ...
  while string.len(molec_formula) > 0 do
    ...
    if string.find(molec_formula, "^%u%l%d+") == 1 then
      table.insert(string_codes, {
        ...
        amount = tonumber(
          string.sub(molec_formula, string.find(molec_formula, "%d+"))
        )
      })
      trim = string.len(tostring(string_codes[#string_codes].amount)) + 1
    --no amount-default 1 --none of the above segments
    --Two letter code with number
    ...
    elseif string.find(molec_formula, "^%u") == 1 then --One letter code without number
      ...
      error(
        "Cannot determine next string code in '" .. (molec_formula or "") .. " of original code " .. orig .. "'. Please report this to the Angel's dev team."
      )
    end
...
local function create_recipe_molecule_icons(
molecules_icon,
  molecules_shift,
  molecules_scale
)
  ...
  for molecule_index, molecule_icon in pairs(molecules_icon) do
    if type(molecule_icon) ~= "table" and get_icons(
      molecule_icon
    ) ~= "__angelsrefining__/graphics/icons/void.png" then
      molecules_icon[molecule_index] =
        util.table.deepcopy(get_icons(molecule_icon))
    end
  end
  ...
  for molecule_index, molecule_icon in pairs(molecules_icon) do
    ...
    elseif type(molecule_icon[1]) ~= "table" then
      ...
      for molecule_icon_layer_index, molecule_icon_layer in
        pairs(molecule_icon)
      do
        if not molecule_icon_layer.icon_size then
          molecules_icon[molecule_index][molecule_icon_layer_index].icon_size =
            32
        end
      end
    end
...
function angelsmods.functions.create_gas_fluid_icon(molecule_icon, tints)
  ...
  if molecule_icon then
    if type(molecule_icon) ~= "table" then
      ...
      if molecule_icon.icon then
        molecule_icon.icon_size =
          molecule_icon.icon_size or molecule_icon[2] or 32
      else
    ...
    molecule_icon.scale =
      molecule_icon.scale or molecule_icon[4] or 15 / molecule_icon.icon_size
  ...
  if tints then
    if type(tints) ~= "table" then
      ...
      tints = {
        top = unify_tint(
          icon_tints_table[(reference[1] or {form = "unknown"}).form][1] or {}
        ),
        mid = unify_tint(
          icon_tints_table[(reference[2] or {form = "unknown"}).form][2] or {}
        ),
        bot = unify_tint(
          icon_tints_table[(reference[3] or {form = "unknown"}).form][3] or {}
        )
      }
    else
...
function angelsmods.functions.create_gas_recipe_icon(
bot_molecules_icon,
  tints,
  top_molecules_icon
)
  ...
  bot_molecules_icon =
    create_recipe_molecule_icons(
      bot_molecules_icon,
      {{-11.5, 12}, {11.5, 12}, {0, 12}},
      10.24 / 32
    )
  top_molecules_icon =
    create_recipe_molecule_icons(
...
function angelsmods.functions.create_liquid_recipe_icon(
bot_molecules_icon,
  tints,
  top_molecules_icon
)
...
function angelsmods.functions.create_viscous_liquid_recipe_icon(
bot_molecules_icon,
  tints,
  top_molecules_icon
)
...
filter_type,
  ...
  local viscous_liquid_fluid_icon =
    angelsmods.functions.create_viscous_liquid_fluid_icon(nil, tints)

  local recipe_icons = {viscous_liquid_fluid_icon[1], {
    -- base layer required for background shadow
    icon = string.format(
      "__angelsrefining__/graphics/icons/filter-%s.png",
      filter_type
    ),
    icon_size = 32,
    scale = 32 / 32 * 0.85
  ...
  table.insert(recipe_icons, {
    icon = string.format(
      "__angelsrefining__/graphics/icons/angels-liquid/filter-%s-front.png",
      filter_type
    ),
    icon_size = 32,
    scale = 32 / 32 * 0.85
...
function angelsmods.functions.create_solid_recipe_icon(
bot_molecules_icon,
  solid_item_name,
  top_molecules_icon
)
...
function angelsmods.functions.get_recipe_tints(layers, opacity)
  ...
  for index, name in pairs(layers) do
    if type(name) == "table" then
      tints[index] = name
      tints[index].a =
        opacity or (tints[index].a and tints[index].a or ((tints[index].r < 1 and tints[index].g < 1) and alpha or alpha * 255))
    --ammend alpha
    --if alpha, maintain
      ...
      for _, type in pairs({"fluid" --[[,"item"]]}) do
        if data.raw[type][name] then
          ...
          tints[index] = {
            ...
            a = base.base_color.a or ((base.base_color.r < 1 and base.base_color.g < 1) and alpha or alpha * 255) --if alpha, maintain
          }
          break
        end
      end
...
local function findHex(hex)
  ...
  local function h(i, j)
    return j and tonumber("0x" .. hex:sub(i, j)) / 255 or tonumber(
      "0x" .. hex:sub(i, i)
    ) / 15
  end
...
local function toColor(color)
  return isColor(color) or findHex(color) or findRGB(color) or {
    r = 0,
    g = 0,
    b = 0,
    a = 1
...
function angelsmods.functions.fluid_color(
chemical_formula --color blending based on a general chemical formula
)
  local color = {}
  ...
  if ave_denom == 2 and ((rgb[1] == icon_tints_table["C"][1] and rgb[2] == icon_tints_table["H"][1]) or (rgb[1] == icon_tints_table["H"][1] and rgb[2] == icon_tints_table["C"][1])) then
    ...
    local function sigmoid(
    x,
      b,
      c --more maxreader madness :D
    )
      return 1 / (1 + 2 ^ (-b * (x + c)))
    end
    ...
    color = {
      r = math.sqrt(red / comb),
      g = math.sqrt(green / comb),
      b = math.sqrt(blue / comb),
      a = 1
    }
...
function angelsmods.functions.flow_color(
chemical_formula --makes it lighter by some margin
)
...
function angelsmods.functions.allow_productivity(recipe_name)
  if data.raw.recipe[recipe_name] then
    for i, module in pairs(data.raw.module) do
      ...
      for i, module_except in
        pairs(angelsmods.refining.productivity_exception)
      do
...
function angelsmods.functions.add_flag(
entity,
  flag -- Adds a flag to an item/fluid (may be a table containing a list of items/fluids)
)
  ...
  for _, type in
    pairs({"item", "tool", "item-with-entity-data", "fluid"}) --list of things to hide
  do
      ...
      if type == "fluid" and flag == "hidden" then --also remove barrel if a fluid
        ...
        for _, void_category in pairs({"water", "chemical"}) do
          angelsmods.functions.OV.disable_recipe(
            "angels-" .. void_category .. "-void-" .. entity
          )
        end
      else
...
function angelsmods.functions.remove_flag(
entity,
  flag_to_remove -- Removes a flag to an item/fluid (may be a table containing a list of items/fluids)
)
  if type(entity) == "table" then
    for _, ent in pairs(entity) do
...
function angelsmods.functions.disable_barreling_recipes(fluid_to_disable)
  angelsmods.functions.OV.disable_recipe(
    "fill-" .. fluid_to_disable .. "-barrel"
  )
  angelsmods.functions.OV.disable_recipe(
    "empty-" .. fluid_to_disable .. "-barrel"
  )
  angelsmods.functions.OV.disable_recipe(
    "fill-" .. fluid_to_disable .. "-liquid-bot"
  )
  angelsmods.functions.OV.disable_recipe(
    "empty-" .. fluid_to_disable .. "-liquid-bot"
  )
      ...
      for kx, fluid in pairs(data.raw.fluid) do
        if fluid.name == icon_name then
          if item.icons then
            ...
            if fluid.icons then
              item.icons = util.combine_icons(item.icons, fluid.icons, {
                scale = 16 / icon_size,
                shift = {0, 5}
              })
            end
...
-- CREATE VOID RECIPES --------------------------------------------------------
-------------------------------------------------------------------------------
function angelsmods.functions.make_void(
fluid_name,
  void_category,
  void_amount -- categories: chemical (fluid, flare-stack)
)
  ...
  -- no valid void object found
  -- fluid voids
  --[[,"water"]] -- no valid void category found
  elseif data.raw.item[fluid_name] then -- item voids
    if void_category == "bio" then
      ...
      void_output_probability = 1 -- no valid void category found
    else
      recipe = nil
    end
  else
  ...
  if recipe then -- valid
    ...
    recipe.always_show_products =
      void_output_amount * void_output_probability > 0
    ...
    recipe.order =
      data.raw["item-group"][data.raw["item-subgroup"][void_input_subgroup].group].order or "z"
    --recipe.order = recipe.order .. "[" .. data.raw["item-subgroup"][void_input_subgroup].group .. "]"
    recipe.order = recipe.order .. "-"
    recipe.order =
      recipe.order .. data.raw["item-subgroup"][void_input_subgroup].order
    --recipe.order = recipe.order .. "[" .. void_input_subgroup .. "]"
    recipe.order = recipe.order .. "-"
    recipe.order =
      recipe.order .. ((data.raw.fluid[fluid_name] or data.raw.item[fluid_name] or {}).order or "z")
    --recipe.order = recipe.order .. "[" .. fluid_name .. "]"
    recipe.order =
      string.len(recipe.order) <= 200 and recipe.order or recipe.order:sub(
        1,
        200
      ) -- order limited to 200 characters
    recipe.icons =
      util.table.deepcopy(
        get_icons(void_output_item) or {
          {icon = "__angelsrefining__/graphics/icons/void.png"}
        }
      )
    recipe.icon_size = 32
...
-------------------------------------------------------------------------------
function angelsmods.functions.make_converter(
fluid_name_other,
  fluid_name_angels
)
  if angelsmods.trigger.enableconverter then
    if data.raw.fluid[fluid_name_angels] and data.raw.fluid[fluid_name_other] then
      ...
      angelsmods.functions.converter_counter =
        angelsmods.functions.converter_counter + 1
      ...
      data:extend({{
        ...
        ingredients = {{
          type = "fluid",
          name = fluid_name_angels,
          amount = 50
        }},
        results = {{
          type = "fluid",
          name = fluid_name_other,
          amount = 50
        }},
        icon_size = 32,
      ...
      if angelsmods.trigger.enableconverter then  -- hide the unused other fluid
      else
...
-------------------------------------------------------------------------------
function angelsmods.functions.add_crafting_category(
crafting_machine_type,
  ...
  local crafting_machine_prototype =
    data.raw[crafting_machine_type][crafting_machine_name]
  crafting_machine_prototype.crafting_categories =
    crafting_machine_prototype.crafting_categories or {}

  for _, category_name in
    pairs(crafting_machine_prototype.crafting_categories)
  do
...
function angelsmods.functions.remove_crafting_category(
crafting_machine_type,
  crafting_machine_name,
  crafting_category
)
  ...
  local crafting_machine_categories =
    data.raw[crafting_machine_type][crafting_machine_name].crafting_categories
  ...
  for category_index, category_name in pairs(crafting_machine_categories) do
    if category_name == crafting_category then
      ...
      if next(crafting_machine_categories) then return
      else
        angelsmods.functions.add_crafting_category(
          crafting_machine_type,
          crafting_machine_name,
...
-------------------------------------------------------------------------------
function angelsmods.functions.set_fast_replace_category(
crafting_machine_type,
  crafting_machine_name,
  next_upgrade
)
  ...
  if not data.raw[crafting_machine_type] then return end
  local crafting_machine1 =
    crafting_machine_name and data.raw[crafting_machine_type][crafting_machine_name]
  if not crafting_machine1 then return end
  local crafting_machine2 =
    next_upgrade and data.raw[crafting_machine_type][next_upgrade]
  ...
  if FRC2 then --change it
    if box_equal(
      crafting_machine1.collision_box,
      crafting_machine2.collision_box
    ) then
    ...
    end -- FRC2==nil --transition 2 to 1?
  else
    crafting_machine2.fast_replaceable_group = FRC1
    angelsmods.functions.set_fast_replace_category(
      crafting_machine_type,
...
-- MODIFY NEXT_UPGRADE --------------------------------------------------------
-------------------------------------------------------------------------------
function angelsmods.functions.set_next_upgrade(
crafting_machine_type,
  crafting_machine_name,
  next_upgrade
)
