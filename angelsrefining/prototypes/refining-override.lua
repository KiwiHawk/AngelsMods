...
data.raw.item["hazard-concrete"].stack_size =
  angelsmods.trigger.pavement_stack_size
data.raw.item["refined-concrete"].stack_size =
  angelsmods.trigger.pavement_stack_size
data.raw.item["refined-hazard-concrete"].stack_size =
  angelsmods.trigger.pavement_stack_size
...
for _, fluid_n in pairs(data.raw.fluid) do
  if not (fluid_n.auto_barrel == false) then
    ...
    if mods["bobplates"] then
      ...
      elseif not string.find(fluid_n.name, "acid") == nil or string.sub(
        fluid_n.name,
        -4
      ) == "acid" then
    ...
    icons = angelsmods.functions.create_liquid_recipe_icon(
      {
        "y-con_water",
        "water-purified",
        mods["angelspetrochem"] and {
          "__angelspetrochem__/graphics/icons/solid-sulfur.png",
          32
        } or "sulfur"
      },
      "WsSS"
...
if data.raw.item["uraninite"] then
  ...
  table.insert(data.raw.technology["slag-processing-1"].effects, {
    type = "unlock-recipe",
    recipe = "slag-processing-up"
  })
  table.insert(data.raw.technology["advanced-ore-refining-3"].effects, {
    type = "unlock-recipe",
    recipe = "angelsore-crystal-mix-up1-processing"
  })
  table.insert(data.raw.technology["advanced-ore-refining-3"].effects, {
    type = "unlock-recipe",
    recipe = "angelsore-crystal-mix-up2-processing"
  })
end
...
if data.raw.item["thorium-ore"] then
  -- pure sorting
  OV.modify_output("angelsore2-pure-processing", {
    "thorium-ore",
    "platinum-ore"
  })
end
