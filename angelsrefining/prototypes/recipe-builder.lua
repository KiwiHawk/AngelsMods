...
local item_subtypes = {
  -- add additional item subtypes here as necessary for recipes
  "item",
  "tool",
  "ammo"
}
...
local function check_ingredients(ingredients)
  ...
  while i <= l do
    ...
    if j then
      ...
      l = l - 1
    -- ingredient is already present, add to existing ingredient
    elseif i_count == 0 then -- delete ingredient on fallback sentinel
      table.remove(ingredients, i)
      l = l - 1
    else
      -- adjust ingredient for fallback
      ingredients[i] = {
        type = i_type,
        name = i_name,
        amount = i_count
...
RB.build = function(
recipe_list -- use like data:extend
)
      ...
      if mn > 0 and mx > 0 then
        ...
        if k then
          if add_list[k].amount then
            ...
            add_list[k].amount_max =
              math.max(add_list[k].amount_max + mx, add_list[k].amount_min)
          end
        else
          add_list[j] = {
            name = n,
      ...
      elseif mn > 0 then
        ...
        if k then
          if add_list[k].amount then
            ...
            add_list[k].amount_min = add_list[k].amount_min + mn
            add_list[k].amount_max =
              math.max(add_list[k].amount_max, add_list[k].amount_min)
          end
        else
          add_list[j] = {
            name = n,
...
local function p_split_key(t, k, e, d)
  local v, v_e, v_n =
    t[k],
    t.expensive and t.expensive[k],
    t.normal and t.normal[k]
  if e then
    if d then
...
local function p_result_merge(target, base, patch, split, e)
  ...
  if split then
    ...
    p, pc, ps =
      p_split_key(patch, "result", e),
      p_split_key(patch, "result_count", e),
      p_split_key(patch, "results", e)
  else
    b, bc, bs = base.result, base.result_count, base.results
    p, pc, ps = patch.result, patch.result_count, patch.results
  end
  if ps then
    local rs = p_merge_item_lists(
      bs or {{
        name = b,
        type = "item",
        amount = bc or 1
  ...
  if base then
    ...
    if patch.normal or base.normal or patch.expensive or base.expensive then
      ...
      for k in pairs(p_splittable) do
        result.normal[k] =
          p_merge(p_split_key(base, k, false), p_split_key(patch, k, false), k)
        result.expensive[k] =
          p_merge(p_split_key(base, k, true), p_split_key(patch, k, true), k)
      end
