...
function angelsmods.settings.hide_setting(
setting_type,
  setting_name,
  forced_setting_value
)
  if data.raw[setting_type] and data.raw[setting_type][setting_name] then
    ...
    if forced_setting_value ~= nil then
      ...
      data.raw[setting_type][setting_name].forced_value =
        data.raw[setting_type][setting_name].default_value
    end
  else
